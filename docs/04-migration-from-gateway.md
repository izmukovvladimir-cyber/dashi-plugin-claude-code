...
Этот документ — для тех, у кого уже работает Python `claude -p` gateway (репо [izmukovvladimir-cyber/jarvis-telegram-gateway](https://github.com/izmukovvladimir-cyber/jarvis-telegram-gateway) или приватный fork `gateway-dashis-agents`), и нужно переехать на плагин до **2026-06-15**.
